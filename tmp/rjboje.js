/* rjboje.js v3 (20260923k) — kutije boja istog modela uz „Odaberi boju:" na stranici bicikla / e-bicikla.
   ...
   Od verzije k (23.09.) i bicikl s jednom bojom dobiva našu kutiju (temin se sakrije). Kad model ima grupu: temin .colorBtn se SAKRIJE (ostaje u DOM-u — tema po broju .colorBtn učitava veličine) i cijeli red
...
(function () {
  ...
  /* i/j: bez bljeska teminog kvadrata na osvježavanju — ako tag nosi data-g="1" (artikl ima grupu, postavlja sweep iz JSON-a)
  ...
  function preHide(on) {
    var st = $('#rjboje-pre'), st0 = $('#rjboje-pre0');   /* pre0 = inline <style> uz tag u sadržaju (sweep), primijenjen pri parsiranju, prije dohvata skripte */
    if (!on) { if (st) st.parentNode.removeChild(st); if (st0) st0.parentNode.removeChild(st0); return; }
  ...
  function go() {
    ...
    Promise.all([kat, map]).then(function (res) {
      ...
      } else if (A[me] && A[me].s) {   /* k (Ivo 23.09.): i bicikl s JEDNOM bojom dobiva našu plain kutiju umjesto teminog hex kvadrata */
        grouped = true; lsAdd(me);
        list = [{ id: me, rec: A[me] }];
      } else {
